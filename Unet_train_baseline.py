...
def train():
    ...
    save_dir = "save_model_baseline"  # 使用不同的保存目录
    ...
    epochs = 50  # 增加到100 epochs，配合Early Stopping
    print(f"\n开始训练...")
    ...
    print(f"  - 数据增强: 仅水平翻转 + 垂直翻转 (基线版本)")
    print(f"  - 最大Epochs: {epochs} (Early Stopping: patience=20)")
    print("="*70)
    ...
    # 保存最终模型（最后一个epoch）
    final_model_path = os.path.join(save_dir, "unet_baseline_final.pth")
