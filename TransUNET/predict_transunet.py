...
def calculate_metrics_local(pred, target, num_classes=2):
    ...
    for i in range(pred.shape[0]):
        ...
        for c in range(num_classes):
            ...
            if p_c.sum() + t_c.sum() == 0: dice = 1.0
            else: dice = 2 * intersection / (p_c.sum() + t_c.sum() + 1e-8)
...
def get_args():
    ...
    # 模型文件路径
    parser.add_argument('--model_path', type=str, required=True, help='训练好的TransUNet模型路径')
    
    # [差异点] TransUNet 特有参数
    parser.add_argument('--vit_name', type=str, default='R50-ViT-B_16', help='ViT模型架构')
    parser.add_argument('--n_skip', type=int, default=3, help='跳跃连接数')
    parser.add_argument('--vit_patches_size', type=int, default=16, help='Patch大小')
    ...
    # 输出参数
    parser.add_argument('--output_dir', type=str, default='./predict_output_transunet', help='预测结果保存目录')
    ...
    parser.add_argument('--eval_on_resize', action='store_true', help='是否在256x256尺寸上计算指标')
...
def load_model(args, device):
    """加载 TransUNet 模型"""
    print(f"加载 TransUNet ({args.vit_name}) 模型...")
    ...
    # 处理 DataParallel 的 module. 前缀 和 checkpoint 字典结构
    ...
    for i, sampled_batch in enumerate(tqdm(test_loader, desc="预测进度")):
        ...
        if args.evaluate_metrics:
            if args.eval_on_resize:
                ...
                pred_final = np.array(pred_resized).astype(int)
                
                acc, miou, mdice, hd95 = calculate_metrics_local(
                    np.expand_dims(pred_final, 0),
                    np.expand_dims(gt_mask_binary, 0),
                    num_classes=args.num_classes
                )
